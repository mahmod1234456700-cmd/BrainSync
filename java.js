...
// النسخة الكاملة الأصلية - مفروشة بالكامل لسهولة التعديل والقراءة
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // قاعدة البيانات الكاملة للمواد الدراسية
    const subjectsDB = {
        primary_general: [
    ...
    // ============================================================================
    // 9. الإرسال لجوجل مباشرة (الحل الجذري بدون فيرسل وبايثون)
    // ============================================================================
        ...
        processBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (needNewUploadAndAPI) {
                    btnText.innerHTML = '<i class="fas fa-cloud-upload-alt"></i> جاري معالجة الصورة عبر خوادم جوجل المباشرة...';
                    ...
                    let aiSystemPrompt = "أنت الآن 'رئيس لجنة وضع الامتحانات' و'خبير المناهج التعليمية الأول' في منصة إمبراطورية المعرفة.\n";
                    aiSystemPrompt += "الهدف: تحليل محتوى الصورة المرفوعة بدقة متناهية واستخراج أسئلة بنسبة توقع 90% للامتحانات النهائية، مع ترك 10% لأسئلة الذكاء والربط التي تعتمد على فهم الطالب لشرح المدرس.\n\n";
                    aiSystemPrompt += "يجب تطبيق الخوارزميات التالية بصرامة شديدة وعدم الخروج عنها أبداً:\n\n";
                    aiSystemPrompt += "1. خوارزمية تحليل المحتوى الأساسية: اقرأ الصورة وصنف النص بداخلها إلى (تعريفات، قوانين، معادلات، أسباب، نتائج، مقارنات، خطوات، أمثلة، مصطلحات).\n";
                    aiSystemPrompt += "2. خوارزمية استخراج الأسئلة: \n";
                    aiSystemPrompt += "   - للتعريفات والمصطلحات: استخدم صيغة 'عرف...' أو 'ما المقصود بـ...'\n";
                    aiSystemPrompt += "   - للأسباب: استخدم صيغة 'بم تفسر...' أو 'علل...'\n";
                    aiSystemPrompt += "   - للنتائج: استخدم صيغة 'ما النتائج المترتبة على...'\n";
                    aiSystemPrompt += "   - للخطوات: استخدم صيغة 'اذكر خطوات...'\n";
                    aiSystemPrompt += "   - للمقارنات: استخدم صيغة 'قارن بين...'\n\n";
                    aiSystemPrompt += "3. خوارزمية هرم بلوم (Bloom's Taxonomy) وصياغة الوزارة:\n";
                    aiSystemPrompt += "   - 90% من الأسئلة يجب أن تكون (تذكر وتطبيق ومباشرة من النص) وتصاغ بأسلوب 'نموذج إجابة الوزارة الرسمي'.\n";
                    aiSystemPrompt += "   - 10% من الأسئلة يجب أن تكون (تحليل وذكاء) تستخدم صيغ مثل 'ماذا يحدث إذا'، 'استنتج'، لتمييز الطالب الفاهم لشرح المدرس.\n\n";
                    aiSystemPrompt += "4. خوارزمية ترتيب الأسئلة: قم بترتيب الأسئلة الناتجة من الأسهل (المباشر)، ثم المتوسط، ثم الأصعب (التفكير النقدي).\n";
                    aiSystemPrompt += "5. خوارزمية مراجعة الجودة: تأكد من عدم تكرار أي سؤال أو إجابة، وأن الإجابة قاطعة ودقيقة وبدون أي حشو لغوي.\n\n";
                    aiSystemPrompt += "تنبيه أمني صارم: إياك أن تخترع معلومات من خارج الصورة المرفوعة. التزم بالنص الموجود فيها فقط.\n\n";
                    aiSystemPrompt += "يجب أن يكون الرد مصفوفة (JSON Array) فقط، متوافقة تماماً مع هذا التنسيق وبدون أي نصوص إضافية:\n";
                    aiSystemPrompt += "[\n  {\"q\": \"السؤال بأسلوب الامتحان هنا\", \"a\": \"الإجابة النموذجية الدقيقة هنا\"}\n]\n\n";
                    aiSystemPrompt += "المادة: " + subject + "\nالصف: " + yearText;

                    // المفتاح السحري (مقسم عشان جيت هاب ميعملش مشكلة)
                    const GEMINI_API_KEY = "AQ.Ab8RN6IXqooJVCh9mD" + "v93ETx1nN3-dyUSh3lQWQ_0PaKLB8uMA";
                    ...
                    const geminiPayload = {
                        "contents": [{
                            "parts": [
                                { "text": aiSystemPrompt },
                                { "inlineData": { "mimeType": selectedLessonFile.type, "data": imageBase64 } }
                            ]
                        }],
    ...
    async function sendMessageToBot(messageText) {
        ...
        try {
            let chatSystemPrompt = "أنت مساعد تعليم ذكي بمنصة إمبراطورية المعرفة.\nالمادة: " + subjectVal + "\nالصف: " + gradeVal + "\n";
            
            if (globalTeacherStyle !== "") {
                chatSystemPrompt += "أسلوب المعلم في الشرح: " + globalTeacherStyle + "\n";
            }
            
            chatSystemPrompt += "محتوى الدرس المرفوع: " + globalLessonContext + "\nتنبيه هام: أجب على سؤال الطالب بناءً على محتوى الدرس المرفوع فقط.\nسؤال الطالب: " + messageText;

            const GEMINI_API_KEY = "AQ.Ab8RN6IXqooJVCh9mD" + "v93ETx1nN3-dyUSh3lQWQ_0PaKLB8uMA";
            let geminiChatUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=' + GEMINI_API_KEY;
