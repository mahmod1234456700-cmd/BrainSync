...
document.addEventListener(
    ...
    lessonImageInput.addEventListener(
        ...
        if (event.target.files.length > 0) {
            ...
            if (!fileCheck.type.includes('image')) {
                ...
                alert("عفواً، مسموح برفع الصور فقط لتوفير مساحة التخزين. يرجى اختيار أو التقاط صورة.");
    ...
    // ============================================================================
    // 9. التحقق، التطهير الأمني (Sanitization)، والاتصال بجوجل مباشرة
    // ============================================================================
    ...
    document.getElementById('process-btn').addEventListener(
        ...
        if (!selectedLessonFile) {
            ...
            alert("يرجى تصوير أو إرفاق صورة لصفحة الكتاب أولاً.");
        ...
        scanningHtml += ' جاري قراءة وتلخيص الصورة المرفوعة بأمان...';
        ...
        try {
            ...
            if (existingData.archived_version) {
                ...
                if (existingData.archived_version.archived_at) {
                    ...
                    if (timeDifference > EIGHTEEN_MONTHS_MS) {
                        ...
                        if (existingData.archived_version.imageUrl) {
                            
                            try {
                                
                                await storage.refFromURL(existingData.archived_version.imageUrl).delete();
                                
                            } catch(e) {
                                
                                console.log("صورة الأرشيف غير موجودة في الاستوريدج.");
            ...
            if (existingData.current_version) {
                ...
                if (existingData.current_version.imageHash === newImageHash) {
                    ...
                    console.log("الصورة متطابقة، تم جلب التلخيص من بنك الأسئلة الحالي بأمان.");
            ...
            if (needNewUploadAndAPI) {
                ...
                let newStoragePath = 'lessons_backup/';
                
                newStoragePath += summaryDocId;
                
                newStoragePath += '_';
                
                newStoragePath += Date.now();
                
                const newStorageRef = storage.ref(newStoragePath);
                
                await newStorageRef.put(selectedLessonFile);
                
                const newImageUrl = await newStorageRef.getDownloadURL();

                // 1. تحويل الصورة إلى نظام يقبله جوجل مباشرة
                ...
                // 2. البرومبت الصارم بتاعك (تم نقله من البايثون)
                let aiSystemPrompt = "أنت الآن بروفيسور ومحاضر عالمي.\n";
                
                aiSystemPrompt += "مهمتك: قراءة النص الموجود في الصورة المرفوعة، وتحليله، واستخراج ملخص شامل بنظام (سؤال وجواب).\n";
                
                aiSystemPrompt += "ملاحظة أمنية: التزم بالنص الموجود في الصورة فقط ولا تضف معلومات خارجية.\n";
                ...
                const GEMINI_API_KEY = "AQ.Ab8RN6LUOZydLpPB4E_qp7xxmvvtV6AhqjjVtoQfdWv5-S6wcQ";
                ...
                let aiJsonData = JSON.parse(aiResponseText);
            ...
            doneHtml += ' تم إنهاء التحليل المأمون بنجاح';
            ...
            setTimeout(
                ...
                let resetBtnHtml = '🚀 تحليل صورة أخرى بأمان';
            ...
            console.error("خطأ أمني أو تقني:", error);
            
            let errHtml = '<i class="fas fa-exclamation-triangle"></i>';
            
            errHtml += ' خطأ في المعالجة الآمنة';
            
            btnText.innerHTML = errHtml;
            
            processBtn.classList.remove('processing');
            
            alert("حدث خطأ أثناء تحليل الصورة، تأكد من جودة الصورة وأنها لصفحة كتاب.");
    ...
    function showOutput(serverData, subjectName) {
        ...
        resultHtml += 'تحليل الصورة المرفوعة لـ: ';
        ...
        resultHtml += '<p style="color: #475569; line-height: 1.6;">تم تحليل الصورة واستخراج المحتوى بنجاح. يمكنك الآن سؤال البروفيسور الذكي في الأعلى عن أي نقطة في هذا الدرس وسيجيبك صوتاً وكتابة.</p>';
    ...
    function generateRealPDF(serverData) {
        ...
        let titleText = 'تحليل الصورة المرفوعة | ';
