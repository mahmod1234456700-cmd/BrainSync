...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================================================================
    // 8. عرض المخرجات وتوليد ملف PDF شامل (بدون إظهار الأسئلة على الشاشة)
    // ============================================================================
    ...
    function generateRealPDF(serverData, subjectName) {
        ...
        serverData.qa_data.forEach((item, index) => {
            ...
            qaHtml += '<div style="margin-bottom: 20px; background: #f8fafc; padding: 16px; border-radius: 8px; border-right: 4px solid #10b981; direction: rtl; text-align: right; position: relative; z-index: 1;">';
            qaHtml += '<p style="color: #0f172a; margin: 0 0 8px 0; font-size: 15px;"><strong>س ' + questionCount + ': ' + cleanQuestion + '</strong></p>';
            
            if (item.options && Array.isArray(item.options) && item.options.length > 0) {
                qaHtml += '<div style="margin: 10px 0; padding: 10px; background: #ffffff; border-radius: 6px; border: 1px solid #cbd5e1;">';
                item.options.forEach((opt) => {
                    qaHtml += '<div style="margin-bottom: 6px; color: #1e293b; font-size: 14px;">• ' + stripParentheses(opt) + '</div>';
                });
                qaHtml += '</div>';
            }

            if (!isExamMode) {
                qaHtml += '<p style="margin: 10px 0 0 0; line-height: 1.8; font-size: 13px; color: #059669;"><strong>الإجابة الصحيحة:</strong><br>' + item.a.replace(/\n/g, '<br>') + '</p>';
                
                if (item.reason) {
                    qaHtml += '<p style="margin: 8px 0 0 0; line-height: 1.8; font-size: 13px; color: #b45309;"><strong>السبب والتفسير الوافي:</strong><br>' + item.reason.replace(/\n/g, '<br>') + '</p>';
                }
            } else {
