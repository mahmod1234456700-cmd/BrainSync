...
// ============================================================================
// التعديل الأمني: دالة تسجيل الدخول مع طلب وفحص الباسورد للإدارة
// ============================================================================
async function handleUserLogin() {
    ...
    try {
        ...
        // التحقق من الباسورد لو الرقم ده رقم إدارة
        if (isAuthorizedAdmin) {
            let savedPassword = "1234"; // الباسورد الافتراضي
            if (doc.exists) {
                teacherData = doc.data();
                savedPassword = teacherData.adminPassword || "1234";
            }

            let enteredPassword = prompt("أدخل الرقم السري الخاص بك كمسؤول:");
            ...
            if (enteredPassword !== savedPassword) {
                alert("الرقم السري غير صحيح! تم طردك ومنع الدخول.");
                document.getElementById('auth-phone').value = '';
                btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> دخول الفوري';
                return;
            }
        ...
        if (doc.exists) {
            ...
            teacherData = {
                ...
                role: assignedRole,
                adminPassword: isAuthorizedAdmin ? "1234" : null, // حفظ الباسورد الافتراضي للإدمن الجديد
                createdAt: new Date()
            };
...
// ============================================================================
// 3. رسائل النجاح، وبناء واجهة المستخدم بعد الدخول (بما فيها تغيير الباسورد)
// ============================================================================
...
// دالة تغيير الرقم السري للإدارة من المنصة مباشرة
async function changeAdminPassword() {
    ...
    try {
        ...
        if (!doc.exists) {
            alert("حدث خطأ: الحساب غير موجود في قاعدة البيانات.");
            return;
        }
        ...
        let oldPass = prompt("أدخل الرقم السري الحالي للإدارة:");
        if (!oldPass) return;

        if (oldPass !== currentSavedPassword) {
            alert("الرقم السري الحالي غير صحيح! تم إلغاء العملية.");
            return;
        }

        let newPass = prompt("أدخل الرقم السري الجديد:");
        if (!newPass || newPass.trim().length < 4) {
            alert("يجب أن يتكون الرقم السري الجديد من 4 أرقام/حروف على الأقل.");
            return;
        }

        let confirmPass = prompt("أعد كتابة الرقم السري الجديد للتأكيد:");
        if (newPass !== confirmPass) {
            alert("كلمتا السر غير متطابقتين! تم إلغاء التغيير.");
            return;
        }
        ...
        showToast("تم تغيير الرقم السري للإدارة بنجاح! 🔑");
