...
// قائمة أرقام الإدارة المعتمدة (تم إضافة 01205599003 لفرز المسؤولين وإظهار الداش بورد)
...
function createAuthScreen() {
    ...
    overlay.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15, 23, 42, 0.85); z-index:999999; display:none; flex-direction:column; align-items:center; justify-content:center; direction:rtl; overflow-y:auto; font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; backdrop-filter: blur(8px); padding: 20px; box-sizing: border-box;';
    ...
    overlay.innerHTML = `
        <div style="width:100%; max-width:440px; background:#ffffff; border-radius:16px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5); overflow:hidden; border-top: 5px solid #3b82f6;">
            ...
            <!-- قسم إرسال إيصال الدفع للتفعيل اليدوي -->
...
async function notifyAdminSubscriptionExpired(expiredPhone, durationText) {
    try {
        const adminAlertMsg = `tنبيه من BrainSync: المشترك رقم (${expiredPhone}) انتهت مدة اشتراكه (${durationText || "المحددة"}) وتم قفل حسابه وإجباره على التجديد.`;
        // تسجيل التنبيه في قاعدة البيانات عشان يظهر للإدارة
        ...
        // إرسال طلب Webhook / API لإرسال رسالة واتساب على رقمك (01205599003 / 01067479440 / 01026336159)
...
async function checkAndLockIfExpired(phone, teacherData) {
    if (AUTHORIZED_ADMIN_PHONES.includes(phone)) return false; // عدم قفل حسابات الإدارة
    ...
    if (teacherData.status === "VIP_Active" && teacherData.subscriptionEnd) {
        ...
        // لو تاريخ النهاية عدى الوقت الحالي
        if (Date.now() > endDate) {
            ...
            // إرسال تنبيه الواتساب للإدارة (مش للعميل)
...
async function handleUserLogin() {
    ...
    try {
        ...
        if (doc.exists) {
            ...
            // فحص هل الاشتراك انتهى وقفل الحساب فوراً
...
// ============================================================================
// 3. رسائل النجاح، وبناء واجهة المستخدم بعد الدخول (مع إظهار الداش بورد للمسؤولين)
// ============================================================================
...
function buildDynamicUserMenu(phone, role) {
    ...
    // فرز الأرقام: يظهر زر الداش بورد فقط للأرقام المعتمدة في مصفوفة المسؤولين
...
// ============================================================================
// 4. الداش بورد الكامل (مع خاصية إضافة أرقام VIP يدوياً مجاناً بايدك)
// ============================================================================
async function loadAndShowDashboard() {
    ...
    container.innerHTML = `
        <!-- الشريط الجانبي الأيمن الكحلي (Navy Blue Sidebar) -->
        <div style="width: 260px; background: linear-gradient(180deg, #0b194f 0%, #060e2b 100%); color: #ffffff; display: flex; flex-direction: column; flex-shrink: 0; box-shadow: -4px 0 15px rgba(0,0,0,0.2);">
            ...
            <div style="padding: 15px 10px; overflow-y: auto; flex: 1;">
                ...
                <a href="javascript:void(0)" class="dash-nav-item active" style="display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #ffffff; text-decoration: none; border-radius: 8px; background: rgba(37, 99, 235, 0.25); border-right: 4px solid #3b82f6; margin-bottom: 5px; font-weight: bold;"><i class="fas fa-users"></i> إدارة الحسابات والتفعيل</a>
                <a href="javascript:void(0)" class="dash-nav-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #cbd5e1; text-decoration: none; border-radius: 8px; margin-bottom: 5px;"><i class="fas fa-book"></i> الدورات والملخصات</a>
                <a href="javascript:void(0)" class="dash-nav-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #cbd5e1; text-decoration: none; border-radius: 8px; margin-bottom: 5px;"><i class="fas fa-sitemap"></i> إدارة التصنيفات</a>
                
                <div style="font-size: 0.75rem; color: #64748b; padding: 15px 15px 5px; font-weight: bold;">المالية والطلبات</div>
                <a href="javascript:void(0)" class="dash-nav-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #cbd5e1; text-decoration: none; border-radius: 8px; margin-bottom: 5px;"><i class="fas fa-shopping-cart"></i> الطلبات وإيصالات الدفع</a>
                <a href="javascript:void(0)" class="dash-nav-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 15px; color: #cbd5e1; text-decoration: none; border-radius: 8px; margin-bottom: 5px;"><i class="fas fa-wallet"></i> التقارير المالية</a>
            </div>
        ...
        <!-- المنطقة الرئيسية اليسرى -->
        <div style="flex: 1; display: flex; flex-direction: column; overflow-y: auto; background: #f8fafc;">
            <!-- الشريط العلوي -->
            <div style="background: #ffffff; padding: 16px 30px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <h2 style="margin: 0; color: #0f172a; font-size: 1.4rem; font-weight: bold;">إدارة الحسابات وطلبات تفعيل VIP</h2>
                    <span style="color: #64748b; font-size: 0.85rem;">مراجعة إيصالات التحويل وإضافة أرقام VIP مجانية يدوياً</span>
                </div>
                <div style="display: flex; align-items: center; gap: 15px;">
                    <button id="dash-refresh-btn" style="background: #e0f2fe; color: #0369a1; border: none; padding: 10px 18px; border-radius: 8px; font-weight: bold; cursor: pointer;"><i class="fas fa-sync-alt"></i> تحديث القائمة</button>
                    <div style="width: 38px; height: 38px; border-radius: 50%; background: #0b194f; color: white; display: flex; align-items: center; justify-content: center; font-weight: bold;">ADMIN</div>
                </div>
            </div>
            ...
            <div style="padding: 25px 30px;">
                ...
                <!-- أيقونة وكارت إضافة رقم VIP يدوياً مجاناً بايدك -->
                <div style="background: #ffffff; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); margin-bottom: 25px; border: 2px solid #3b82f6;">
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                        ...
                        <input type="text" id="manual-add-duration" value="سنة كاملة (مجاني)" placeholder="المدة (مثال: شهر، سنة، مجاني)..." style="width: 200px; padding: 12px 15px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; text-align: center;">
                        <button id="manual-add-btn" style="background: #2563eb; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 8px;"><i class="fas fa-plus-circle"></i> إضافة وتفعيل VIP مجاني</button>
                    </div>
                </div>
                ...
                <!-- جدول إدارة المستخدمين والتفعيل اليدوي -->
                    ...
                    <table style="width: 100%; border-collapse: collapse; text-align: right;">
                        <thead>
                            <tr style="background: #f1f5f9; color: #334155; border-bottom: 2px solid #e2e8f0;">
                                ...
                                <th style="padding: 15px 18px;">إجراء التفعيل اليدوي</th>
                            </tr>
                        </thead>
    ...
    // تشغيل زرار إضافة رقم VIP يدوياً مجاناً
    document.getElementById('manual-add-btn').addEventListener('click', async () => {
        ...
        let durationVal = document.getElementById('manual-add-duration').value.trim() || "سنة كاملة (مجاني)";
        ...
        let daysToAdd = 365;
        if (durationVal.includes("شهر") || durationVal.includes("30")) daysToAdd = 30;
        else if (durationVal.includes("6") || durationVal.includes("ست")) daysToAdd = 180;
        else if (durationVal.includes("سنة") || durationVal.includes("عام")) daysToAdd = 365;
...
// دالة جلب البيانات وعرض إحصائيات وأزرار التفعيل اليدوي
async function loadDashboardTableData() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            // فحص هل الحساب منتهي الصلاحية أثناء العرض
            if (status === "VIP_Active" && data.subscriptionEnd) {
            ...
            let currentDurationText = data.vipDurationText || "شهر واحد";
            ...
            rowsHtml += `
                <tr style="border-bottom: 1px solid #f1f5f9; transition: 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='#ffffff'">
                    ...
                    <td style="padding: 16px 18px;">
                        <input type="text" id="duration_${phone}" value="${currentDurationText}" placeholder="مثال: شهر، 6 شهور، سنة..." style="width: 170px; padding: 8px 12px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.9rem;">
                    </td>
                    <td style="padding: 16px 18px; color: #64748b; font-size: 0.9rem;">${subDate}</td>
                    <td style="padding: 16px 18px;">
                        <button onclick="manualActivateVIP('${phone}')" style="background: #10b981; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 0.9rem; margin-left: 5px;"><i class="fas fa-check"></i> تفعيل VIP يدوي</button>
                        <button onclick="deactivateUserVIP('${phone}')" style="background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; padding: 8px 12px; border-radius: 6px; cursor: pointer; font-size: 0.85rem;">إلغاء</button>
                    </td>
                </tr>
            `;
        });
...
// دالة التفعيل اليدوي VIP بواسطة المدة المكتوبة يدوياً مع حساب تاريخ القفل التلقائي
window.manualActivateVIP = async function(phone) {
    ...
    let customDurationText = durationInput ? durationInput.value.trim() : "شهر واحد";
    if (!customDurationText) customDurationText = "شهر واحد";

    let daysToAdd = 30;
    if (customDurationText.includes("سنة") || customDurationText.includes("عام")) daysToAdd = 365;
    else if (customDurationText.includes("6") || customDurationText.includes("ست")) daysToAdd = 180;
    else if (customDurationText.includes("3") || customDurationText.includes("تلات")) daysToAdd = 90;
    else daysToAdd = 30;
...
// ============================================================================
// 5. نظام المحاولات المجانية للزوار (وحظر أصحاب الاشتراكات المنتهية)
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showOutput(serverData, subjectName) {
        ...
        resultHtml += '<h4 style="color: #0f172a; margin-top: 0; border-bottom: 2px solid #3b82f6; display: inline-block; padding-bottom: 5px;">BrainSync - تحليل المادة لـ: ' + (serverData.grade || "") + ' - ' + subjectName + '</h4>';
        resultHtml += '<p style="color: #475569; line-height: 1.7; font-weight: bold;">تم تلخيص المحتوى واستخراج كافة الأسئلة (المقالية، الاختياري، والصح والخطأ مع ذكر الأسباب) بنفس لغة النص الأصلي.</p>';
    ...
    function generateRealPDF(serverData, subjectName) {
        ...
        document.getElementById('pdf-header-title').innerText = 'ملف BrainSync | ' + (serverData.subjectTitle || subjectName) + ' | ' + (serverData.grade || "");
        ...
        const opt = {
            margin: 0.5,
            filename: 'BrainSync_' + (serverData.subjectTitle || subjectName).replace(/\s+/g, '_') + '_' + Date.now() + '.pdf',
