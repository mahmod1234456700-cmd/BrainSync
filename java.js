...
// تصميم عصري فخم (Modal) لإدخال الرقم السري للإدارة بدلاً من رسالة المتصفح القبيحة
function openAdminPasswordModal(callback) {
    ...
    modal.innerHTML = `
        <div style="background:#ffffff; width:100%; max-width:420px; border-radius:16px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.4); overflow:hidden; border-top: 5px solid #2563eb;">
            <div style="padding:22px 20px; background:#f8fafc; border-bottom:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center;">
                <div style="display:flex; align-items:center; gap:10px;">
                    <div style="width:38px; height:38px; background:#eff6ff; color:#2563eb; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:1.1rem;"><i class="fas fa-shield-alt"></i></div>
            ...
            <div style="padding:25px 20px;">
                ...
                <div style="display:flex; gap:12px;">
                    <button id="admin-pass-submit" style="flex:1; background:#2563eb; color:white; border:none; padding:13px; border-radius:10px; font-weight:bold; cursor:pointer; font-size:1rem; box-shadow:0 4px 12px rgba(37,99,235,0.3);"><i class="fas fa-check-circle"></i> دخول</button>
...
async function handleUserLogin() {
    ...
    const currentIP = await fetchDeviceIP(); // التقاط الـ IP وتخزينه لكل مستخدم
    ...
    try {
        ...
        if (isAuthorizedAdmin) {
            ...
            // استخدام النافذة المنبثقة الفخمة بدلاً من prompt القديم
            openAdminPasswordModal(async (enteredPassword) => {
                ...
                if (enteredPassword !== savedPassword) {
                    alert("الرقم السري غير صحيح! تم رفض الدخول.");
                    document.getElementById('auth-phone').value = '';
                    btn.innerHTML = '<i class="fas fa-sign-in-alt"></i> دخول الفوري';
                    return;
                }

                // متابعة عملية الدخول بعد نجاح التحقق
                await finishLoginProcess(teacherRef, doc, teacherData, phone, currentIP, deviceFingerprint, isAuthorizedAdmin, assignedRole, btn);
            });
            return;
        }
...
async function finishLoginProcess(teacherRef, doc, teacherData, phone, currentIP, deviceFingerprint, isAuthorizedAdmin, assignedRole, btn) {
    if (doc.exists) {
        if (!isAuthorizedAdmin) {
            teacherData = doc.data();
        }
        ...
        if (teacherData.status === "VIP_Active" || isAuthorizedAdmin) {
            // تخزين وتحديث الـ IP لكل جهاز يفتح المنصة
...
// تصميم عصري فخم لتغيير الرقم السري للإدارة من المنصة
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showOutput(serverData, subjectName) {
        ...
        resultHtml += '<button id="native-print-btn" class="download-pdf-btn" style="background:#2563eb; margin-bottom: 10px;"><i class="fas fa-file-pdf"></i> تحميل / حفظ التلخيص كملف PDF على التليفون</button></div>';
